module Requests exposing (authConfig, authHeader, buildUrl, decoderComment, decoderCommentResponse, decoderCredentials, decoderFullUser, decoderHomeData, decoderLeaderboard, decoderPrizeData, decoderPuzzleDetail, decoderPuzzleMini, decoderPuzzleSet, decoderPuzzleStats, decoderRegisterResponse, decoderSendEmailResponse, decoderSubmissionData, decoderSubmissionResponse, decoderThemeData, decoderTooSoonSubmit, decoderUserStats, decoderVideoLink, encodeComment, encodeCredentials, encodeEmail, encodeRegister, encodeSubmission, encodeToken, getHomeDataPublic, getHomeDataUser, getLeaderboard, getNoAuth, getPrizeList, getPuzzlePublic, getPuzzleUser, getStatsPublic, getStatsUser, getWithAuth, noAuthConfig, noInputString, postComment, postLogin, postNoAuth, postRegister, postSendEmail, postSubmit, postWithAuth)
...
decoderHomeData =
    Decode.map4 HomeData
        ...
        (Decode.field "num_submissios" Decode.int)


decoderUserStats : Decoder UserStats
decoderUserStats =
    Decode.map7 UserStats
        (Decode.field "username" Decode.string)
        (Decode.field "num_submit" Decode.int)
        (Decode.field "num_solved" Decode.int)
        (Decode.field "points" Decode.int)
        (Decode.field "rank" Decode.int)
        (Decode.field "num_prizes" Decode.int)
        (Decode.field "submissions" <| Decode.maybe (Decode.list decoderSubmissionData))
        ...
        (Decode.field "leaderboard"
            (Decode.list
                (Decode.map2 PuzzleLeaderboardUnit
                    (Decode.field "username" Decode.string)
                    (Decode.field "submissionDatetime" Iso8601.decoder)
                )
            )
        )
...
postComment authToken puzzleId string =
    postWithAuth authToken [ "puzzles", String.fromInt puzzleId, "comment" ] CommentGotResponse decoderCommentResponse (encodeComment string)
